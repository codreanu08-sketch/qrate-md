...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      await registerWithConfirmation(email, password, lang, acceptedMarketing);
      setEmailSent(true);
    } catch (err: any) {
      console.error('EROARE REGISTER:', err);
      setError(err.message || (lang === 'ru' ? 'Ошибка регистрации. Попробуйте еще раз.' : 'Eroare la înregistrare. Încearcă din nou.'));
      setLoading(false);
    }
  };
        ...
        {emailSent ? (
          <div className="bg-white/80 backdrop-blur-xl border border-white rounded-[2.5rem] shadow-2xl shadow-emerald-100/50 p-10 text-center">
            ...
            <h2 className="text-2xl font-black text-slate-900 uppercase tracking-tighter mb-3">
              {lang === 'ru' ? 'Verificati emailul' : 'Verifică emailul'}
            </h2>
            <p className="text-slate-500 text-sm leading-relaxed mb-1">
              {lang === 'ru' ? 'Am trimis un link de activare la' : 'Am trimis un link de activare la'}
            </p>
            <p className="font-black text-slate-900 text-sm mb-5">{email}</p>
            <p className="text-slate-400 text-xs leading-relaxed mb-8">
              {lang === 'ru'
                ? 'Apasa pe link pentru a-ti activa contul. Verifica si folderul Spam daca nu apare in inbox.'
                : 'Apasă pe link pentru a-ți activa contul. Verifică și folderul Spam dacă nu apare în inbox.'}
            </p>
            <Link href={`/${lang}/auth/login`}
              className="inline-block text-[10px] font-black text-blue-600 uppercase tracking-widest hover:underline">
