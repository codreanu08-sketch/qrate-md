...
export default function LocationsPage() {
  ...
  const handleCreateCompanyInline = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: createError } = await supabase.from('companies').insert([{ name: newCompanyName.trim(), owner_id: user.id }]).select().single();
  ...
  const downloadQR = (id: string, name: string, locLogo: string) => {
    ...
    if (!qrCanvas) { alert(locale === 'ru' ? "Ошибка при генерации QR-кода." : "Eroare la generarea codului QR."); return; }
    ...
    const buildCanvas = () => {
      try {
        ...
        ctx.fillStyle = "#64748b"; ctx.font = "italic 40px sans-serif";
        ...
        ctx.strokeStyle = "#f1f5f9"; ctx.lineWidth = 4; ctx.beginPath();
      ...
      <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-8 flex items-center justify-center font-sans text-slate-900">
        <div className="max-w-md w-full bg-white p-8 md:p-12 rounded-[3.5rem] shadow-[0_20px_60px_rgba(0,0,0,0.03)] border border-slate-100 text-center">
          ...
          <form onSubmit={handleCreateCompanyInline} className="space-y-4">
            ...
            <button disabled={isCreatingCompany} className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-sm uppercase tracking-[0.2em] hover:bg-blue-600 transition-all shadow-xl flex items-center justify-center gap-3 disabled:opacity-50">
              {isCreatingCompany ? <Loader2 className="animate-spin" /> : (locale === 'ru' ? 'Сохранить и Продолжить' : 'Salvează și Continuă')}
            </button>
          </form>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <form onSubmit={handleAddLocation} className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-xl border border-slate-100 mb-12">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" placeholder={t('form.placeholder_name')} value={newName} onChange={(e) => setNewName(e.target.value)}
                  className="w-full bg-slate-50 rounded-2xl py-3.5 px-5 font-bold text-slate-800 outline-none focus:ring-2 focus:ring-blue-500 border border-transparent" required />
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-black text-slate-500 mb-1.5 flex items-center gap-1.5">
                  ...
                  {locale === 'ru' ? 'Ссылка Google Reviews (необязательно)' : 'Link Google Reviews (opțional)'}
                </label>
              ...
              <div>
                ...
                <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-dashed border-slate-200">
                  ...
                  <button type="button" onClick={() => fileInputRef.current?.click()}
                    className="bg-slate-900 text-white px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-wider hover:bg-slate-800 disabled:opacity-50 transition-colors flex-1" disabled={uploading}>
          ...
          <button type="submit" disabled={isAdding || !newName.trim() || !companyId}
            ...
            {isAdding ? (<><Loader2 size={18} className="animate-spin" />{locale === 'ru' ? "Добавление..." : "Se adaugă..."}</>) : t('form.submit_btn')}
          </button>
        </form>
              ...
              return (
                <div key={loc.id} onClick={() => setSelectedLocationId(isSelected ? null : loc.id)}
                  ...
                  <div className="bg-slate-50 p-6 rounded-[2.5rem] mb-6 self-center border border-transparent shadow-inner">
                    {qrUrl && <QRCodeCanvas value={qrUrl} size={140} level="H" imageSettings={loc.logo_url ? { src: loc.logo_url, height: 34, width: 34, excavate: true } : undefined} />}
                  </div>
        ...
        <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-xl border border-slate-100">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 border-b border-slate-100 pb-4">
            ...
            {selectedLocationId && (
              <button onClick={() => setSelectedLocationId(null)} className="flex items-center gap-1.5 text-xs font-black uppercase bg-slate-100 hover:bg-slate-200 text-slate-600 px-4 py-2 rounded-xl transition-colors">
                  ...
                  <div className="space-y-1.5 w-full sm:w-auto">
                    <div className="flex items-center gap-3 flex-wrap">
                      <div className="flex gap-0.5 text-amber-500">
                        {[...Array(5)].map((_, i) => <Star key={i} size={14} fill={i < review.rating ? "currentColor" : "none"} className={i < review.rating ? "text-amber-500" : "text-slate-200"} />)}
                      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-2xl max-w-sm w-full border border-slate-100 text-center">
            ...
            <div className="flex gap-3">
              ...
              <button type="button" onClick={() => deleteLocation(showDeleteModal.id)} className="flex-1 bg-rose-600 hover:bg-rose-700 text-white font-black py-3 rounded-xl uppercase text-xs tracking-wider transition-colors shadow-lg shadow-rose-100">{t('delete_modal.confirm')}</button>
            </div>
          </div>
        </div>
      )}
