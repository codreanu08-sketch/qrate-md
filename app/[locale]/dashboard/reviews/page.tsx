      ...
      <div className="p-4 md:p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          ...
          <div className="flex items-center gap-1.5 flex-wrap justify-end">
            {rev.is_recovery_win && (
              <span className="text-[9px] font-black bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full uppercase">🏆 Win</span>
            )}
...
export default function AllReviewsDashboard() {
  ...
  // Follow-ups necesare — recenzii negative cu telefon
          ...
          <>
            ...
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 mb-8">
              ...
              <div className="mt-4 flex justify-end">
                <button onClick={() => { setSelectedLocation('all'); setSelectedEmployee('all'); setTimeFilter('all'); setCustomDate(''); }}
            ...
            {followUpsNeeded.length > 0 && (
              <div className="bg-rose-50 border border-rose-200 rounded-3xl p-5 mb-8">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <p className="font-black text-sm text-rose-800 uppercase tracking-tight">
                      {locale === 'ru' ? `Follow-up necesar (${followUpsNeeded.length})` : `Follow-up necesar (${followUpsNeeded.length})`}
                    </p>
                    <p className="text-[10px] text-rose-500 font-medium">
                      {locale === 'ru' ? 'Clienți nemulțumiți cu telefon — contactează-i pe WhatsApp' : 'Clienți nemulțumiți cu telefon — contactează-i pe WhatsApp'}
                    </p>
                  </div>
                </div>
                <div className="space-y-2">
                      ...
                      <div key={rev.id} className="flex items-center justify-between gap-3 bg-white rounded-2xl p-3 border border-rose-100">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="flex gap-0.5 shrink-0">
                            {[1,2,3,4,5].map(i => <Star key={i} size={10} className={i <= rev.rating ? 'text-rose-400 fill-rose-400' : 'text-slate-200 fill-slate-200'} />)}
                          </div>
                          <div className="min-w-0">
                            <p className="text-xs font-black text-slate-800 truncate">{rev.full_name || (locale === 'ru' ? 'Anonim' : 'Anonim')}</p>
                  ...
                  {followUpsNeeded.length > 5 && (
                    <p className="text-[10px] text-rose-500 font-bold text-center pt-1">
                      +{followUpsNeeded.length - 5} {locale === 'ru' ? 'mai mulți' : 'mai mulți'}
                    </p>
                  )}
                </div>
              </div>
      ...
      {activePhoto && (
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={() => setActivePhoto(null)}>
          <div className="relative max-w-3xl w-full max-h-[85vh] bg-white rounded-2xl overflow-hidden shadow-2xl" onClick={e => e.stopPropagation()}>
            <button onClick={() => setActivePhoto(null)} className="absolute top-4 right-4 bg-slate-950/60 text-white p-2 rounded-full hover:bg-slate-950 transition-colors z-10">
              <X size={18} />
            </button>
