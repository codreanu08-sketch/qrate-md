    ...
    <div className="min-h-screen bg-[#F8FAFC] p-4 md:p-12 font-sans text-slate-900">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center mb-12 gap-6">
          <Link href="/dashboard/employees" className="group flex items-center gap-3 text-slate-400 font-black text-xs uppercase tracking-[0.2em] hover:text-slate-900 transition-all w-full md:w-auto justify-center md:justify-start">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto">
            {/* MODIFICAT AICI: Am scos hidden md:flex ca să fie vizibil și pe mobil */}
            <div className="flex flex-wrap justify-center bg-white p-2 rounded-[2rem] shadow-sm border border-slate-100 gap-1 sm:gap-0 w-full sm:w-auto">
               {['7d', '1m', 'custom'].map(p => (
                <button 
                  key={p} 
                  onClick={() => setPeriod(p)}
                  className={`px-6 sm:px-8 py-3 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest transition-all flex-1 sm:flex-none text-center ${period === p ? 'bg-slate-900 text-white shadow-xl' : 'text-slate-400 hover:text-slate-600'}`}
                >
                  {t(`periods.${p}`)}
                </button>
              ))}
        ...
        {loading ? (
          ...
          <div ref={reportRef} className="space-y-10 p-2 md:p-6 rounded-[3rem]">
            ...
            <div className="bg-white rounded-[3rem] md:rounded-[4rem] p-6 md:p-10 shadow-[0_30px_100px_rgba(0,0,0,0.04)] border border-slate-50 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-80 h-80 bg-blue-50 rounded-full blur-[100px] -mr-40 -mt-40 opacity-60" />
              
              <div className="flex flex-col lg:flex-row items-center gap-8 md:flex-row lg:gap-12 relative z-10">
                <div className="relative group">
                  <div className="absolute inset-0 bg-blue-500 blur-3xl opacity-10" />
                  {employee?.photo_url ? (
                    <img src={employee.photo_url} crossOrigin="anonymous" className="w-44 h-44 md:w-56 md:h-56 rounded-[3rem] md:rounded-[4rem] object-cover border-[10px] border-white shadow-2xl relative z-10" alt={employee.name} />
                  ) : (
                    <div className="w-44 h-44 md:w-56 md:h-56 rounded-[3rem] md:rounded-[4rem] bg-slate-50 border-[10px] border-white flex items-center justify-center text-slate-200 shadow-2xl relative z-10">
                      <User size={60} />
                    </div>
                  )}
                  <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-blue-600 text-white px-6 md:px-8 py-2.5 rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] shadow-xl border-4 border-white z-20 whitespace-nowrap">
                    {t('badge_elite')}
                  </div>
                </div>

                <div className="flex-1 text-center lg:text-left mt-4 lg:mt-0 w-full">
                  <div className="flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-6 mb-4">
                    <h1 className="text-3xl md:text-5xl lg:text-6xl font-[900] text-slate-900 uppercase tracking-tighter leading-none break-words">
                  ...
                  <p className="text-slate-400 font-bold uppercase tracking-[0.4em] text-[10px] md:text-[11px] mb-8 lg:mb-10 flex flex-wrap items-center justify-center lg:justify-start gap-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 md:gap-6">
                    <div className="bg-slate-50/50 p-6 rounded-[2.5rem] border border-slate-100">
                      ...
                      <p className="text-3xl md:text-4xl font-black text-slate-900">{reviews.length}</p>
                    </div>
                    <div className="bg-slate-50/50 p-6 rounded-[2.5rem] border border-slate-100">
                      ...
                      <p className="text-3xl md:text-4xl font-black text-slate-900">{reviews.filter(r => r.rating >= 4).length}</p>
                    </div>
                    <div className="bg-slate-50/50 p-6 rounded-[2.5rem] border border-slate-100">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12 items-start">
              ...
              <div className="lg:col-span-2 space-y-6 md:space-y-8">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2 px-4 md:px-6">
                  <h2 className="font-black text-xs uppercase tracking-[0.4em] text-slate-400">{t('reviews_title')}</h2>
                  {period === 'custom' && (
                    <input type="date" value={specificDate} onChange={e => setSpecificDate(e.target.value)} className="bg-white border border-slate-200 rounded-xl px-4 py-2 font-black text-[11px] text-blue-600 outline-none focus:border-blue-500 shadow-sm w-full sm:w-auto" />
                  )}
                </div>

                {reviews.length === 0 ? (
                  <div className="bg-white p-16 md:p-24 rounded-[3rem] md:rounded-[4rem] border border-dashed border-slate-200 text-center">
                  ...
                  reviews.map((rev) => {
                    ...
                    return (
                      <div key={rev.id} className="bg-white p-6 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] border border-slate-50 shadow-sm hover:shadow-xl transition-all duration-500">
                        
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6 pb-4 border-b border-slate-50">
                          <div className="flex items-center gap-3">
                            <div className="w-12 h-12 bg-gradient-to-br from-slate-100 to-slate-200 text-slate-700 rounded-2xl flex items-center justify-center font-black uppercase text-base border border-slate-200 shadow-sm flex-shrink-0">
                              {(rev.full_name || 'A')[0]}
                            </div>
                            <div>
                              <h3 className="font-black text-slate-900 text-base md:text-lg leading-tight">
                          ...
                          <div className="bg-slate-50 px-5 py-2 rounded-full text-[10px] font-black text-slate-400 uppercase tracking-widest self-start sm:self-auto">
                            {new Date(rev.created_at).toLocaleDateString('ro-RO', { day: '2-digit', month: 'long' })}
                          </div>
                        </div>

                        <p className="text-slate-600 text-lg md:text-xl font-medium leading-[1.8] italic mb-6 pl-1 break-words">
                        ...
                        {rev.phone && (
                          <div className="pt-4">
                            <a
                              ...
                              className="flex items-center justify-center gap-2 text-xs md:text-sm font-black uppercase tracking-wider text-emerald-700 bg-emerald-50 hover:bg-emerald-600 hover:text-white border border-emerald-200 px-5 py-3 rounded-xl transition-all text-center w-full"
                            >
              ...
              <div className="space-y-6 md:space-y-8 lg:sticky lg:top-8">
                <div className="bg-slate-900 text-white p-6 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl">
                  <h3 className="font-black text-[10px] uppercase tracking-[0.3em] mb-8 md:mb-10 text-blue-400">{t('distribution_title')}</h3>
                ...
                <div className="bg-blue-600 p-8 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] text-white shadow-xl flex flex-col items-center text-center relative overflow-hidden">
                   <div className="w-16 h-16 md:w-20 md:h-20 bg-white/10 rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center mb-6 md:mb-8 backdrop-blur-xl border border-white/20 z-10">
                    <Award size={36} />
                  </div>
                  <h4 className="font-black text-xs uppercase tracking-[0.3em] mb-4 z-10 text-blue-100">{t('performance_status')}</h4>
                  <div className="text-4xl md:text-5xl font-black mb-4 z-10 tracking-tighter">
